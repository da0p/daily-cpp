...
public:
  ...
  InstanceCounter(InstanceCounter const&) noexcept { ++counter_; }
...
public:
  ...
  friend std::ostream& operator<<(std::ostream& os, Window const& w)
  {
...
WindowDemo()
{
  ...
  int const windowCount = std::uniform_int_distribution<>(2, 10)(gen);
  ...
  for(int i = 0; i < windowCount; ++i) {
    ...
    auto r2 = distrib(gen);
    auto name =
      std::string{adjs[(r + i) % adjs.size()]} + nouns[r2 % nouns.size()];
