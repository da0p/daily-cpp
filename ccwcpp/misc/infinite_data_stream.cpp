#include <coroutine>
#include <iostream>
#include <memory>
#include <vector>

template<typename T>
struct Generator
{
  struct promise_type;
  using handle_type = std::coroutine_handle<promise_type>;

  Generator(handle_type h)
    : coro(h)
  {
    std::cout << "Generator::Generator"
              << "\n";
  }
  handle_type coro;

  ~Generator()
  {
    std::cout << "Generator::~Generator"
              << "\n";
    if(coro) {
      coro.destroy();
    }
  }
  Generator(Generator const&) = delete;
  Generator& operator=(Generator const&) = delete;
  Generator(Generator&& other)
    : coro(other.coro)
  {
    other.coro = nullptr;
  }
  Generator& operator=(Generator&& other)
  {
    coro = other.coro;
    other.coro = nullptr;
    return *this;
  }
  int getNextValue()
  {
    std::cout << "Generator::getNextValue"
              << "\n";
    coro.resume();
    return coro.promise().current_value;
  }
  struct promise_type
  {
    promise_type()
    {
      std::cout << "promise_type::promise_type"
                << "\n";
    }
    ~promise_type()
    {
      std::cout << "promise_type::~promise_type"
                << "\n";
    }
    std::suspend_always initial_suspend()
    {
      std::cout << "promise_type::initial_suspend"
                << "\n";
      return {};
    }
    std::suspend_always final_suspend() noexcept
    {
      std::cout << "promise_type::final_suspend"
                << "\n";
      return {};
    }
    auto get_return_object()
    {
      std::cout << "promise_type::get_return_obejct"
                << "\n";
      return Generator{handle_type::from_promise(*this)};
    }
    std::suspend_always yield_value(int value)
    {
      std::cout << "promise_type::yield_value"
                << "\n";
      current_value = value;
      return {};
    }
    void return_void() {}
    void unhandled_exception() { std::exit(1); }
    T current_value;
  };
};

template<typename Cont>
Generator<typename Cont ::value_type>
getNext(Cont cont)
{
  std::cout << "getNext: start"
            << "\n";
  for(auto c : cont) {
    std::cout << "getNext: before co_yield"
              << "\n";
    co_yield c;
    std::cout << "getNext: after co_yield"
              << "\n";
  }
}

int
main(int argc, char* argv[])
{
  std::vector<int> x = {1, 2, 3, 4, 5};
  auto genInt = getNext(x);
  for(int i = 0; i <= 2; ++i) {
    auto val = genInt.getNextValue();
    std::cout << "main: " << val << "\n";
  }

  std::string y = "Hello World";
  auto genString = getNext(y);
  for(int i = 0; i <= 2; ++i) {
    auto val = genString.getNextValue();
    std::cout << "main: " << val << "\n";
  }
}